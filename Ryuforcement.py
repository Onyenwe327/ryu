...
class stateProcessor(object):
	def __init__(self): #Create an sess of image transformation
		with tf.compat.v1.variable_scope("process"): # Image size 200 256
			self.input_state = tf.compat.v1.placeholder(shape=[200, 256, 3], dtype=tf.uint8, name="input_process")
			self.output = tf.image.rgb_to_grayscale(self.input_state)
			self.output = tf.image.crop_to_bounding_box(self.output, 28, 0, 158, 256)
			self.output = tf.image.resize(self.output, [128, 128], method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
...
class Network():
	...
	def _build_model(self):
		...
		self.tf_st = tf.compat.v1.placeholder(dtype=tf.float32, shape=[None, self.height, self.width, self.channel], name='tf_image')
		self.tf_target = tf.compat.v1.placeholder(dtype=tf.float32, shape=[None], name='tf_target')
		self.tf_action = tf.compat.v1.placeholder(dtype=tf.int32, shape=[None], name='tf_action')

		# 32 filter with a size of 8 by 8 pixels, that move  4 pixels
		conv1 = tf.compat.v1.layers.conv2d(self.tf_st, filters=32, kernel_size=8, strides=4, activation=tf.nn.relu)
		conv2 = tf.compat.v1.layers.conv2d(conv1, filters=64, kernel_size=4, strides=2, activation=tf.nn.relu)
		conv3 = tf.compat.v1.layers.conv2d(conv2, filters=64, kernel_size=3, strides=1, activation=tf.nn.relu)
		...
		self.output = tf.compat.v1.layers.dense(fc1, 19, activation=None) # <- 64 doit devenir le nombre d'action possible
		...
		self.error = tf.reduce_mean(tf.math.squared_difference(self.action_value, self.tf_target))
	...
	with tf.compat.v1.Session() as sess:
		...
		for i in range(num_episode):
			...
			if best_epi_r < mean_epi_r:
				...
				saver.save(tf.compat.v1.get_default_session(), chkpnt_path)
			...
			if len_rp_memory > rp_memory_init_size:
				for i in range(100):
					...
					targets_batch = reward_batch + np.invert(done_batch).astype(np.float32) * 0.99 * q_values_next_target[np.arange(size_batch), t_best_actions]
